...
<summary style="cursor: pointer; font-weight: bold; color: #0066cc; list-style: none; padding-left: 0;">
▶ View all v3.x releases
</summary>
<ul style="margin-top: 10px;">
{% for version in site.data.versions.release %}
{% if version contains 'v3' %}
   <li style="display: inline;"><a href="{{ site.baseurl }}/{{ version }}">{{ version }}</a>{% if version == site.data.versions.release[0] %}<span style="color: #28a745; font-weight: bold;"> (current)</span>{% endif %}</li>
   {% unless forloop.last %}{% if version contains 'v3' %}<br>{% endif %}{% endunless %}
{% endif %}
{% endfor %}
</ul>
</details>
