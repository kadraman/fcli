...
<summary style="display: block; list-style: none; cursor: pointer; font-weight: bold; color: #0066cc;">
...
{% if version contains 'v3' %}
   <li><a href="{{ site.baseurl }}/{{ version }}">{{ version }}</a>{% if version == site.data.versions.release[0] %}<span style="color: #28a745; font-weight: bold; white-space: nowrap;"> (current)</span>{% endif %}</li>
{% endif %}
{% endfor %}
</ul>
</details>
